...
public class Main {
	public static void main(String[] args) throws FileNotFoundException, IOException{
		...
		for(Double snr: snratios)
			System.out.println(terminal.calculateThreshold(snr));
		for(int j=1;j<=3;j++){
			System.out.println("Sequenza"+j);
			for(int i=1;i<=4;i++){
				System.out.println("Osservazione"+i+"snr"+snratios.get(i-1));
				DiscreteSignal signal= new SignalLoader("resources/Sequenze/Sequenza_"+j+"/output_"+i+".dat").loadSignal();
				System.out.println(terminal.checkPUSignalPresence(signal, snratios.get(i-1)));
				System.out.println();
			}
		}
	}
